...
def query(context, connection, table, index_name=None, index_value=None):
    meta = MetaData()
    engine = get_engine()
    ...
    for row in result:
        # convert the row into a dictory
...
def insert(context, connection, table, data):
    meta = MetaData()
    engine = get_engine()
    r_table = Table(table, meta, autoload=True, autoload_with=engine)
    stmt = r_table.insert()
...
def delete(context, connection, table, index_name, index_value):
    meta = MetaData()
    engine = get_engine()
    r_table = Table(table, meta, autoload=True, autoload_with=engine)
...
def update(context, connection, table, index_name, index_value, data):
    meta = MetaData()
    engine = get_engine()
    r_table = Table(table, meta, autoload=True, autoload_with=engine)
...
def user_get_all(context):
    ...
    with conn.begin() as trans:
        # user table
        users = query(context, conn, 'user')
        # local_user table
        local_users = query(context, conn, 'local_user')
        # password table
        passwords = query(context, conn, 'password')
...
def user_get(context, user_id):
    ...
    with conn.begin() as trans:
        # user table
        users = query(context, conn, 'user', 'id', user_id)
        if not users:
            raise exception.UserNotFound(user_id=user_id)
        result['user'] = users[0]
        # local_user table
        local_users = query(context, conn, 'local_user', 'user_id', user_id)
        if not local_users:
            raise exception.UserNotFound(user_id=user_id)
        ...
        if result['local_user']:
            result['password'] = query(context, conn, 'password', 'local_user_id',
...
def user_create(context, payload):
    ...
    conn = get_connection()
    with conn.begin() as trans:
        insert(context, conn, 'user', users)

        # ignore auto generated id
        for local_user in local_users:
            local_user.pop('id', None)
        insert(context, conn, 'local_user', local_users)

        inserted_local_users = query(context, conn, 'local_user', 'user_id',
        ...
        insert(context, conn, 'password', passwords)
...
def user_update(context, user_id, payload):
    ...
    with conn.begin() as trans:
        ...
        if table in payload:
            user = payload[table]
            update(context, conn, table, 'id', user_id, user)
        ...
        if table in payload:
            ...
            # ignore auto generated id
            local_user.pop('id', None)
            update(context, conn, table, 'user_id', user_id, local_user)
            updated_local_users = query(context, conn, table, 'user_id',
                                        new_user_id)
            ...
            table = 'password'
            if table in payload:
                delete(context, conn, table, 'local_user_id',
...
def project_get_all(context):
    ...
    with conn.begin() as trans:
        # project table
        projects = query(context, conn, 'project')
...
def project_get(context, project_id):
    ...
    with conn.begin() as trans:
        # project table
        projects = query(context, conn, 'project', 'id', project_id)
        if not projects:
...
def project_create(context, payload):
    ...
    conn = get_connection()
    with conn.begin() as trans:
        insert(context, conn, 'project', projects)
...
def project_update(context, project_id, payload):
    ...
    with conn.begin() as trans:
        ...
        if table in payload:
            ...
            update(context, conn, table, 'id', project_id, project)
        ...
        if project_id != new_project_id:
            ...
            update(context, conn, table, 'target_id', project_id, assignment)
...
def role_get(context, role_id):
    ...
    with conn.begin() as trans:
        # role table
        roles = query(context, conn, 'role', 'id', role_id)
        if not roles:
...
def role_create(context, payload):
    ...
    conn = get_connection()
    with conn.begin() as trans:
        insert(context, conn, 'role', roles)
...
def role_update(context, role_id, payload):
    ...
    with conn.begin() as trans:
        ...
        if table in payload:
            role = payload[table]
            update(context, conn, table, 'id', role_id, role)
        ...
        if role_id != new_role_id:
            ...
            update(context, conn, table, 'role_id', role_id, assignment)
...
def revoke_event_get_all(context):
    ...
    with conn.begin() as trans:
        ...
        revoke_events = query(context, conn, 'revocation_event')
...
def revoke_event_get_by_audit(context, audit_id):
    ...
    with conn.begin() as trans:
        # revocation_event table
        revoke_events = query(context, conn, 'revocation_event', 'audit_id', audit_id)
...
def revoke_event_get_by_user(context, user_id, issued_before):
    ...
    with conn.begin() as trans:
        # revocation_event table
        events = query(context, conn, 'revocation_event', 'user_id', user_id)
...
def revoke_event_create(context, payload):
    ...
    conn = get_connection()
    with conn.begin() as trans:
        insert(context, conn, 'revocation_event', revoke_events)
...
def revoke_event_delete_by_audit(context, audit_id):
    conn = get_connection()
    with conn.begin() as trans:
        delete(context, conn, 'revocation_event', 'audit_id', audit_id)
...
def revoke_event_delete_by_user(context, user_id, issued_before):
    ...
    with conn.begin() as trans:
        ...
        delete(context, conn, 'revocation_event', 'id', event_id)
