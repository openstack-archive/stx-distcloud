...
class User(base.Resource):
    ...
    def __init__(self, manager, id, domain_id, default_project_id,
                 ...
                 extra={}, resource_option_mapper={},
                 nonlocal_user=None, federated_users=None):
        self.manager = manager
        self.id = id
        self.domain_id = domain_id
        ...
        self.extra = extra
        self.resource_option_mapper = resource_option_mapper
        self.local_user = local_user
        self.nonlocal_user = nonlocal_user
        self.federated_users = federated_users
...
class identity_manager(base.ResourceManager):
    ...
    def users_list(self, url):
        ...
        for json_object in json_objects:
            # skip non local users
            if not json_object['local_user']:
                continue
            passwords = []
            for object in json_object['local_user']['passwords']:
                # skip empty password
                if not object:
                    continue
                password = Password(
            ...
            user = User(
                ...
                default_project_id=json_object['default_project_id'],
                enabled=json_object['_enabled'],
                created_at=json_object['created_at'],
                last_active_at=json_object['last_active_at'],
                extra=json_object['extra'],
                resource_option_mapper=json_object['_resource_option_mapper'],
                local_user=local_user,
                nonlocal_user=json_object['nonlocal_user'],
                federated_users=json_object['federated_users'])
